    ...
    if (!$errors) {
        try {
            if ($action === 'hide') {
                ...
                redirect(admin_url('post_comments.php'));
            }
...
$search = get_value('q');
$params = [];
$whereParts = ["comments.target_type = 'post'"];

if ($search !== '') {
    $whereParts[] = "(comments.content LIKE :q OR users.username LIKE :q OR users.display_name LIKE :q OR posts.content LIKE :q)";
    $params['q'] = '%' . $search . '%';
}
...
$totalComments = (int)(db_fetch_one(
    "SELECT COUNT(*) AS total
     ...
     LEFT JOIN posts ON comments.target_id = posts.id
     $where",
    $params
)['total'] ?? 0);
...
<head>
    <meta charset="utf-8">
    <title>Commentaires des annonces — Administration — <?= e(APP_NAME) ?></title>
...
<body>
    ...
    <main class="container page">
        ...
        <section class="card">
            <h1>Commentaires des annonces</h1>
            <p class="muted">
                Modérez les commentaires publiés sur les annonces.
            </p>

            <div class="form-actions">
                <a class="button-secondary" href="<?= e(admin_url('index.php')) ?>">Retour admin</a>
                <a class="button-secondary" href="<?= e(admin_url('posts.php')) ?>">Voir les annonces</a>
            </div>
        </section>
        ...
        <section class="card">
            <form method="get" action="">
                ...
                <input
                    ...
                    name="q"
                    value="<?= e($search) ?>"
                    placeholder="Commentaire, annonce, auteur..."
                >

                <div class="form-actions">
                    <button type="submit" class="button-primary">Rechercher</button>
                    <a class="button-secondary" href="<?= e(admin_url('post_comments.php')) ?>">Réinitialiser</a>
                </div>
            </form>
        </section>
                        ...
                        <tbody>
                            <?php foreach ($comments as $comment): ?>
                                <tr>
                                    ...
                                    <td>
                                        <?php if ($comment['post_content'] !== null): ?>
                                            <a href="<?= e(url('discussion.php?post_id=' . (int)$comment['target_id'] . '#comments')) ?>">
                                                <?= e(excerpt((string)($comment['post_content'] ?: 'Annonce'), 100)) ?>
                                            </a>
                                        <?php else: ?>
                                            Annonce supprimée #<?= e((string)$comment['target_id']) ?>
                                        <?php endif; ?>
                                    </td>
                                    <td>
